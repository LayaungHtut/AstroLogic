...
{#if open}
	<div class="fixed inset-0 z-50 flex items-center justify-center p-4">
		...
		<div
			class="relative z-10 flex max-h-[85vh] w-full max-w-lg flex-col gap-6 overflow-y-auto rounded-2xl bg-surface-container-lowest p-6 shadow-2xl"
		>
			<div class="flex items-center justify-between">
				<div class="flex items-center gap-3">
					<div
						class="flex h-10 w-10 items-center justify-center rounded-xl bg-primary-container text-primary"
					>
			...
			<div class="flex flex-col gap-3">
				<span class="font-mono-data text-[11px] tracking-wider text-on-surface-variant uppercase"
					>{$t('profile.zodiacSign')}</span
				>
				<div class="grid grid-cols-4 gap-2">
					{#each ZODIAC_SIGNS as sign}
						...
						<button
							...
							class="flex flex-col items-center gap-1.5 rounded-xl p-3 text-center transition-all {selectedSign ===
							sign
								? 'bg-primary-container/20 shadow-md ring-2 ring-primary'
								: 'bg-surface-container-high hover:bg-surface-container'}"
						>
							<span
								class="text-2xl"
								style:color={selectedSign === sign ? color : 'var(--color-on-surface-variant)'}
								>{ZODIAC_SYMBOLS[sign]}</span
							>
							<span
								class="font-mono-data max-w-full truncate text-[10px] font-medium text-on-surface"
								>{zInfo.name}</span
							>
						</button>
					{/each}
			...
			<div class="flex justify-end gap-3 pt-2">
				<button
					type="button"
					onclick={() => (open = false)}
					class="rounded-xl px-4 py-2 text-sm font-medium text-on-surface-variant transition-colors hover:bg-surface-container-high"
				>
					{$t('common.cancel')}
				</button>
				<button
					...
					class="rounded-xl bg-primary px-5 py-2 text-sm font-medium text-on-primary shadow-md transition-all hover:bg-primary/90 disabled:cursor-not-allowed disabled:opacity-50"
				>
					{$t('profile.save')}
				</button>
			</div>
