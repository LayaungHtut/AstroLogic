	...
	const filteredBrowseCards = $derived(
		(suitFilter === 'all' ? allCards : allCards.filter((c) => cardSuit(c) === suitFilter)).filter(
			(c) => {
				...
				return c.name.toLowerCase().includes(q) || trans.name.toLowerCase().includes(q);
			}
		)
	);
...
<div class="flex flex-col gap-4">
	<!-- SELECTED CARDS TRAY -->
	<div
		class="rounded-xl border border-primary/20 bg-surface-container-high/40 p-4 backdrop-blur-md"
	>
		<div class="mb-3 flex items-center justify-between">
			...
			<span
				class="font-mono-data rounded-full px-2 py-0.5 text-xs font-bold {atMax
		...
		{:else}
			<div class="flex flex-col gap-1.5">
				{#each selected as sel, i (sel.name)}
					...
					<div
						class="flex items-center gap-2 rounded-lg border border-white/5 bg-surface-container-lowest/80 px-3 py-2 shadow-sm"
					>
						<span
						...
						<span class="flex-1 truncate text-sm font-medium text-on-surface"
							>{cardTrans.name || sel.name}</span
						>
						<button
							type="button"
							class="font-mono-data rounded-full px-2 py-0.5 text-[10px] tracking-wider uppercase transition-colors
								{sel.is_reversed
	...
	<div class="flex flex-wrap items-center justify-between gap-2">
		<div
			class="flex items-center gap-1 rounded-full border border-white/5 bg-surface-container-high/60 p-1"
		>
			<button
				type="button"
				class="font-mono-data flex items-center gap-1.5 rounded-full px-3 py-1 text-xs transition-all {mode ===
				'mystery'
		...
		{#if mode === 'mystery'}
			<button
				type="button"
				class="font-mono-data flex items-center gap-1.5 rounded-full border border-secondary/30 bg-secondary/15 px-3 py-1 text-xs text-secondary transition-all hover:bg-secondary/25 active:scale-95"
				onclick={reshuffle}
	...
	{:else if mode === 'mystery'}
		...
		<div class="flex flex-col gap-2">
			<p class="text-center text-[11px] text-on-surface-variant/80 italic">
				{$locale === 'my'
			...
			<div
				class="grid max-h-120 grid-cols-3 gap-2.5 overflow-y-auto p-1 perspective-[1000px] sm:grid-cols-5"
			>
				{#each mysterySlots as slot, idx (slot.card.card + idx)}
					...
					<button
						type="button"
						class="group relative flex cursor-pointer flex-col items-center transition-transform duration-300 select-none hover:scale-105 active:scale-95 disabled:cursor-not-allowed"
						onclick={() => onMysteryCardClick(idx)}
						disabled={!picked && atMax}
						aria-label={slot.revealed
							? cardTrans.name || slot.card.name
							: `Mystery Card #${slot.id}`}
					>
						<div
							class="relative aspect-2/3 w-full overflow-hidden rounded-xl border shadow-md transition-all duration-500 {picked
								? 'ring-2 shadow-primary/30 ring-primary'
								: 'border-white/10 hover:border-secondary/40'}"
						>
							{#if slot.revealed}
								<!-- FACE UP REVEALED CARD -->
								<div class="absolute inset-0 flex flex-col bg-surface-container-highest">
									{#if slot.card.image}
										<img
									...
									{:else}
										<div
											class="flex h-full w-full items-center justify-center bg-gradient-to-br from-purple-900 to-indigo-950 p-2 text-center text-[10px] text-purple-200"
										>
											{cardTrans.name || slot.card.name}
										</div>
									{/if}
									{#if picked}
										<div
											class="absolute inset-0 flex items-center justify-center bg-primary/20 backdrop-blur-[0.5px]"
										>
											<span class="material-symbols-outlined text-2xl text-white drop-shadow"
												>check_circle</span
											>
										</div>
									{/if}
							...
							{:else}
								...
								<div
									class="absolute inset-0 flex flex-col items-center justify-center overflow-hidden rounded-xl border-2 border-amber-400/30 bg-gradient-to-b from-indigo-950 via-purple-950 to-slate-950 p-2"
								>
									<!-- Background ornate image with gradient fallback -->
									<img
										src={CARD_BACK_URL}
										alt="Card back"
										class="absolute inset-0 h-full w-full object-cover opacity-75 transition-opacity group-hover:opacity-90"
										loading="lazy"
									...
									<div
										class="relative z-10 flex flex-col items-center justify-center gap-1 rounded-full border border-amber-300/30 bg-black/50 p-2.5 backdrop-blur-sm transition-transform group-hover:scale-110"
									>
										<span class="material-symbols-outlined text-xl text-amber-300">sparkles</span>
										<span class="font-mono-data text-[10px] font-bold text-amber-200"
											>#{slot.id}</span
										>
									</div>

									<!-- Top/bottom celestial badges -->
									<div class="pointer-events-none absolute top-1.5 text-[10px] text-amber-300/60">
										✦
									</div>
									<div
										class="pointer-events-none absolute bottom-1.5 text-[10px] text-amber-300/60"
									>
										✦
									</div>
								</div>
						...
						<span
							class="mt-1 line-clamp-1 text-center text-[11px] font-medium {slot.revealed
								? 'text-primary'
								: 'text-on-surface-variant'}"
						>
							{slot.revealed
								? cardTrans.name || slot.card.name
								: `${$locale === 'my' ? 'ကတ် ' : 'Card '}${slot.id}`}
						</span>
					</button>
				{/each}
			</div>
	...
	{:else}
		...
		<div class="flex flex-col gap-3">
			<div class="flex flex-wrap items-center gap-2">
				...
				<div class="flex items-center gap-1.5 overflow-x-auto">
					{#each suits as s}
						<button
							type="button"
							class="font-mono-data shrink-0 rounded-full px-2.5 py-1 text-[10px] tracking-wider uppercase transition-all
								{suitFilter === s.id
			...
			{:else}
				<div
					class="grid max-h-96 grid-cols-3 gap-2 overflow-y-auto pr-1 sm:grid-cols-4 md:grid-cols-6"
				>
					{#each filteredBrowseCards as card (card.card)}
						...
						>
							...
							<span class="line-clamp-1 text-[10px] text-on-surface-variant"
								>{cardTrans.name || card.name}</span
							>
						</button>
					{/each}
				</div>
